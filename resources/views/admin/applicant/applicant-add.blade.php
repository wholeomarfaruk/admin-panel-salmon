        ...
        <div class="main-content-wrap">
            <div class="flex items-center flex-wrap justify-between gap20 mb-27">
                ...
                <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                    ...
                    <li>
                        <div class="text-tiny">New Applicant</div>
                    </li>
                </ul>
            </div>
            <!-- new-applicant -->
            <div class="wg-box">
                <form class="form-new-product form-style-1 needs-validation" action="{{ route('admin.job.applicant.store') }}"
                    ...
                    <fieldset class="name">
                        ...
                        <textarea class="flex-grow @error('message') is-invalid @enderror" placeholder="Message"
                            name="message" tabindex="0" aria-required="true" required autocomplete="message" autofocus>{{ old('message') }}</textarea>
                        @error('message')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('cv_link') is-invalid @enderror" type="text"
                            ...
                            value="{{ old('cv_link') }}" required autocomplete="cv_link" autofocus
                            >
                        @error('cv_link')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                    ...
                    <fieldset class="name">
                        ...
                        <select class="@error('status') is-invalid @enderror" name="status" required>
                            ...
                            <option value="1" {{ old('status') == '1' ? 'selected' : '' }}> Accepted
                            </option>
                            <option value="2" {{ old('status') == '2' ? 'selected' : '' }}> Rejected
                            </option>
                        </select>
                        @error('status')
                            <span class="invalid-feedback" role="alert">
                    ...
                    <fieldset class="name">
                        ...
                        <select class=" @error('job_id') is-invalid @enderror" name="job_id" required>
                            <option value="">Select Job</option>
                            @foreach ($jobs as $job)
                            <option value="{{ $job->id }}"
                                {{ old('job_id') == $job->id ? 'selected' : '' }}> {{ $job->title }}
                            </option>
                            @endforeach
                        </select>
