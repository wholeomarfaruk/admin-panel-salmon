...
@section('content')
    ...
    <div class="main-content-inner">
        <div class="main-content-wrap">
            ...
            <div class="wg-box">
                <form class="form-new-product form-style-1 needs-validation" action="{{ route('admin.popup.store') }}"
                    ...
                    <fieldset class="name">
                        ...
                        <select class=" @error('is_popup_show') is-invalid @enderror" name="is_popup_show" required>
                            <option value="0" {{ old('is_popup_show') == 0 ? 'selected' : '' }}>No</option>
                            <option value="1" {{ old('is_popup_show') == 1 ? 'selected' : '' }}> Yes
                            </option>
                        </select>
                        @error('is_popup_show')
                    ...
                    <fieldset class="name">
                        ...
                        <select id="url_type" class=" @error('url_type') is-invalid @enderror" name="url_type" required>
                            ...
                            <option value="whatsapp" {{ old('url_type') == 'whatsapp' ? 'selected' : '' }}>Whatsapp</option>
                            <option value="phone" {{ old('url_type') == 'phone' ? 'selected' : '' }}> Phone</option>
                            <option value="link" {{ old('url_type') == 'link' ? 'selected' : '' }}> Link</option>
                        </select>
                        @error('url_type')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                    ...
                    <fieldset class="name" id="url-box" style="display:none">
                        ...
                        <input id="url" class="flex-grow @error('url') is-invalid @enderror" type="text" placeholder="url"
                            name="url" tabindex="0" aria-required="true" value="{{ old('url') }}"
